...
function hexRgba(hex,a) { 
  try { return `rgba(${parseInt(hex.slice(1,3),16)},${parseInt(hex.slice(3,5),16)},${parseInt(hex.slice(5,7),16)},${a})`; } 
  catch { return hex; } 
}
...
async function loadDataFromSheets(){
  ...
  try {
    ...
    // API 호출 (에러 방지를 위해 템플릿 리터럴 대신 문자열 덧셈 사용)
    ...
    document.getElementById('headerSub').textContent = 'PJ CODE: 24188 | ' + dateRangeStr + ' | ' + MEMBERS.length + '명 | ' + RAW.length + '건';
...
function updateUI() {
  ...
  yLabels.forEach((label, i) => {
    ...
    if (window.filterStartIndex <= yearEnd && window.filterEndIndex >= yearStart) label.classList.add('active'); 
...
function buildSidebarMembers(){
  const wrap = document.getElementById('sidebarMembers'); 
  ...
  MEMBERS.forEach(name => {
    ...
    card.className = 'member-card-side';
    card.innerHTML = `
...
function drawOvertimeChart(wrapId, d) {
  ...
  const dateMap = {}; 
  d.forEach(r => { dateMap[r.date] = (dateMap[r.date] || 0) + r.min; });
  ...
  CH[wrapId] = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        ...
        datalabels: { 
          display: true, color: '#3d3d5c', anchor: 'end', align: 'right', font: { weight: 'bold', size: 11 }, 
          formatter: v => v > 0 ? v + '일' : '' 
        }
      },
      scales: {
...
function renderCVAnalysis() {
  ...
  all.forEach(r => { dailyTotalMins[r.date] = (dailyTotalMins[r.date]||0) + r.min; });
  ...
  let validCVs = []; 
  cvData.forEach(pt => { if (pt.y !== null) validCVs.push(pt.y); });
  ...
  const weekBubbleData = weeks.map(w => {
      ...
      const dailyT = {}; 
      dWk.forEach(r => { dailyT[r.date] = (dailyT[r.date]||0) + r.min; });
      const vals = [0,0,0,0,0]; 
      Object.values(dailyT).forEach((m, idx) => { if(idx<5) vals[idx]=m; });
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: { 
            backgroundColor: 'rgba(61,61,92,.95)', 
            callbacks: { 
              label: c => ['전체 (' + c.raw.wk.split('-')[1] + ')', '다양성(1-HHI): ' + c.raw.x.toFixed(2), 'Shannon: ' + c.raw.y.toFixed(2), 'CV: ' + c.raw.cvRaw.toFixed(2)] 
            } 
          }
        },
        scales: {
...
function renderLine() {
  ...
  if(leg) leg.innerHTML = MEMBERS.map(n => `<div class="legend-item"><div class="legend-dot" style="background:${MC[n]}"></div>${n}</div>`).join('');
...
function dashClickSub(sub) {
  ...
  if(G.selectedMember) {
    H.dpSub = H.dashSub;
    renderNewDpTop3Highlight();
    renderDpStackedHighlight();
  }
}

function renderNewDpTop3Highlight() {
  // Stub for compatibility with dashClickSub
}
...
function drawHeat(data, year, tableId, color) {
  ...
  const hasOver = Array.from({length:5}, () => new Array(12).fill(false));
  let maxV = 0;
  ...
  mat.forEach(row => row.forEach(v => { if(v > maxV) maxV = v; }));
  if(!maxV) maxV = 1;
...
function renderTimeline() {
  ...
  if(tl) tl.innerHTML = MEMBERS.map(n => `<div class="legend-item"><div class="legend-dot" style="background:${MC[n]}"></div>${n}</div>`).join('');
...
function getRepCardHTML(name, d, tot, pct, mos, pos, color) {
  ...
  const moMap = {}; 
  d.forEach(r => { const mo = r.date.slice(0,7); moMap[mo] = (moMap[mo]||0) + r.min; });
...
function renderDetailPanel(name){
  ...
  const filteredQs = [...new Set(d.map(r => getQFromDate(r.date)))].sort(); 
  ...
  if (currentQ) {
      ...
      const diffs = []; 
      const allSubs = new Set([...Object.keys(curMins), ...Object.keys(prevMins)]);
      allSubs.forEach(s => { 
      ...
      options:{
        ...
        plugins:{
          ...
          datalabels:{
            ...
            formatter:(val,ctx)=>{const p = Math.round(val/catTotal*100); return p > 5 ? cats2[ctx.dataIndex] + '\n' + p + '%' : '';},
  ...
  d.forEach(r => { if(isFocusedWork(r.cat, r.sub)) focusMin += r.min; else distMin += r.min; });
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor:'rgba(61,61,92,.95)',
          callbacks:{
            label: (c) => c.dataset.label === '주요일정' ? c.dataset.customEvts[c.dataIndex].map(e => e.date + ': ' + e.title) : c.dataset.label + ': ' + formatNum(c.parsed.y) + 'h'
          }
        },
        datalabels: {
          ...
          font: { weight: (ctx) => H.dpSub === null ? 'normal' : '800', size: 11 }
        }
      },
      scales: {
...
function renderPatternAnalysis(d, mos) {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor:'rgba(61,61,92,.95)',
            callbacks: {
              label: (c) => [c.raw.moStr + ' (' + c.raw.wk.split('-')[1] + ')', '다양성 지수(1-HHI): ' + c.raw.x.toFixed(2), 'Shannon: ' + c.raw.y.toFixed(2), '총 투입: ' + c.raw.time + 'h']
            }
          },
          datalabels: { display: false }
        },
  ...
  const workDates = Array.from(new Set(d.map(r=>r.date))).sort(); 
  ...
  if(wrap) {
    wrap.innerHTML = `<div style="display:flex; flex-direction:column; gap:8px; height:100%; justify-content:center;">` + 
      ...
      <div style="background:#fff; border:1px solid #eef0f8; border-radius:10px; padding:10px 14px; display:flex; align-items:center; gap:10px;">
        ...
        <div style="flex:1;"><div style="font-size:12px; font-weight:800; color:#3d3d5c;">${c.sub}</div><div style="font-size:10px; color:#888; margin-top:2px;">${c.start} ~ ${c.end}</div></div>
        <div style="text-align:right;"><div style="font-size:16px; font-weight:900; color:${['#f59e0b', '#94a3b8', '#cd7c3b', '#64748b', '#64748b'][i]};">${c.streak}<span style="font-size:10px; color:#aaa; font-weight:normal; margin-left:2px;">일 연속</span></div></div>
      </div>`).join('') + `</div>`;
  }
}
...
function renderEvalDonut(hl){
  ...
  if(top3wrap) {
    top3wrap.innerHTML = top.slice(0,3).map(([sub,min],i) => `
...
function renderEvalTrendChart(hlLabel = null){
  ...
  if(legendEl) {
    legendEl.innerHTML = RADAR_LABELS.map((lbl, i) => `
      <div class="legend-item" style="opacity:${hlLabel!==null&&hlLabel!==lbl?'0.35':'1'}; font-weight:${hlLabel===lbl?'800':'normal'};" onclick="renderEvalTrendChart('${hlLabel===lbl?null:lbl}')">
    ...
    options: {
      ...
      onClick: (e, els) => { 
        if(els.length > 0 && els[0].datasetIndex < RADAR_LABELS.length) {
          renderEvalTrendChart(hlLabel === RADAR_LABELS[els[0].datasetIndex] ? null : RADAR_LABELS[els[0].datasetIndex]); 
        }
      }
    }
  });
...
function renderEvalRadarDual(){
  ...
  const radarOpts = (label, data, bc) => ({
    ...
    options: {
      layout: { padding: 16 }, responsive: true, maintainAspectRatio: false,
...
function renderEvalProfile(){
  ...
  const filteredItemAvgs = RADAR_LABELS.map((lbl, i) => ({ 
    lbl, 
    avg: (() => { 
      const vals = qs.map(q => { const ev = EVAL_DATA.find(e => e.q === q && e.name === name); return ev ? ev.vals[i] : null; }).filter(v => v !== null); 
      return vals.length > 0 ? parseFloat((vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(2)) : 0; 
    })(), 
    idx: i 
  }));
      ...
      <div class="eval-scores-list">
        ${filteredItemAvgs.map((item,i) => `
          <div class="eval-score-item">
            ...
            <span class="eval-score-item-val" style="color:${item.avg>=4.5?'#22c55e':item.avg>=3.5?'#00458c':item.avg>=2.5?'#f97316':'#ef4444'};">${item.avg.toFixed(1)}</span>
          </div>
        `).join('')}
      </div>
      ...
      <div class="eval-top3-row">
        ${sorted.slice(0,3).map((item,rank) => `
          <div class="eval-top3-card" style="background:${['#fef9ee','#f8fafc','#fdf6f0'][rank]};border:1.5px solid ${['#f59e0b','#94a3b8','#cd7c3b'][rank]}33;">
            ...
            <div class="eval-top3-score" style="color:${item.avg>=4?'#22c55e':item.avg>=3?'#00458c':'#f97316'};">${item.avg.toFixed(1)}</div>
            ${getTrend(item.idx)}
          </div>
        `).join('')}
      </div>
      ...
      RADAR_LABELS.map((lbl,i) => { 
        ...
        return `
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;">
            ...
            <span style="font-size:10px;font-weight:700;color:${diff > 0 ? '#22c55e' : diff < 0 ? '#ef4444' : '#aaa'};margin-left:auto;">${diff > 0 ? `▲+${diff.toFixed(1)}` : diff < 0 ? `▼${Math.abs(diff).toFixed(1)}` : '–'}</span>
          </div>
        `; 
      }).join('');
  }
...
function renderCorrelationChartInit() {
  ...
  if(tabsWrap) {
    tabsWrap.innerHTML = RADAR_LABELS.map(lbl => 
      `<button class="tab-btn-sm ${lbl === activeCorrelationMetric ? 'active' : ''}" onclick="updateCorrelationMetric('${lbl}')">${lbl}</button>`
    ).join('');
  }
