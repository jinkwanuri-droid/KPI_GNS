// 전역 에러 핸들러 추가 (오류 발생 시 화면에 빨간 글씨로 표시)
...
function getRepCardHTML(name, d, tot, pct, mos, pos, color) {
  ...
  const moMap = {}; 
  d.forEach(function(r) { const mo = r.date.slice(0,7); moMap[mo] = (moMap[mo]||0) + r.min; }); 
  const pkM = Object.entries(moMap).sort(function(a,b){ return b[1]-a[1]; })[0];
  
  const dateMap = grpMin(d, 'date'); 
  const pkD = Object.entries(dateMap).sort(function(a,b){ return b[1]-a[1]; })[0];
  
  const subMap = grpMin(d, 'sub'); 
  const top3Subs = Object.entries(subMap).sort(function(a,b){ return b[1]-a[1]; }).slice(0,3);
  
  const stageBarsHTML = CAT_ORDER.filter(function(c){ return catMap[c]>0; }).map(function(c){ 
...
function renderDetailPanel(name){
  const color=MC[name];
  const all=filtered();
  const totalAllMin=all.reduce(function(s,r){ return s+r.min; },0); 
  const d=all.filter(function(r){ return r.name===name; });
  const tot=d.reduce(function(s,r){ return s+r.min; },0);
  ...
  const mos=getMos(d); 
  const subMap=grpMin(d,'sub'); 
  const allYearsSet = new Set(mos.map(function(m){ return m.split('-')[0]; }));
  ...
  const allMemberData = RAW.filter(function(r) { return r.name === name; }); 
  const filteredQsSet = new Set(d.map(function(r) { return getQFromDate(r.date); }));
  ...
  if (currentQ) {
      ...
      allMemberData.forEach(function(r) { 
      ...
      const inc = diffs.filter(function(x) { return x.diff > 0; }).sort(function(a,b) { return b.diff - a.diff; }).slice(0,3); 
      const dec = diffs.filter(function(x) { return x.diff < 0; }).sort(function(a,b) { return a.diff - b.diff; }).slice(0,3);
      ...
      const buildMiniList = function(arr, isInc) {
          ...
          return arr.map(function(x) { 
            return '<div style="display:flex; justify-content:space-between; align-items:center; padding:6px 0; border-bottom:1px solid #f0f1f5;"><span style="font-size:11px; font-weight:700; color:#3d3d5c; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:120px;">' + x.sub + '</span><div style="display:flex; align-items:center; gap:6px;"><span style="font-size:10px; color:#aaa;">' + x.prev.toFixed(1) + 'h → ' + x.cur.toFixed(1) + 'h</span><span style="font-size:11px; font-weight:800; color:' + (isInc?'#ef4444':'#3b82f6') + ';">' + (isInc?'▲':'▼') + ' ' + Math.abs(x.diff).toFixed(1) + 'h</span></div></div>'; 
          }).join('');
      };
  ...
  const catMap=grpMin(d,'cat'); 
  const cats2=CAT_ORDER.filter(function(c){ return catMap[c]>0; }); 
  const catData=cats2.map(function(c){ return mToH(catMap[c]||0); }); 
  const catColors=cats2.map(function(c){ return CAT_COLORS[CAT_ORDER.indexOf(c)]; }); 
  const catTotal=catData.reduce(function(a,b){ return a+b; },0);
    ...
    CH.dpFocusBar = new Chart(fBar.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            backgroundColor: 'rgba(61,61,92,.95)',
            callbacks: { label: function(c) { return c.dataset.label + ': ' + c.raw.toFixed(1) + '% (' + (c.datasetIndex === 0 ? formatHour(focusMin) : formatHour(distMin)) + 'h)'; } }
          },
          datalabels: {
  ...
  if(hytw) { 
    ...
    allYears.forEach(function(y) { 
      ...
      b.onclick = function() { 
        G.dpHeatYear = y; 
        hytw.querySelectorAll('.tab-btn-sm').forEach(function(x) { x.classList.remove('active'); }); 
        b.classList.add('active'); 
...
function renderDpCvChart(name, d) {
  ...
  const datesSet = new Set(d.map(function(r){ return r.date; }));
  ...
  const dailyMins = {}; 
  d.forEach(function(r) { dailyMins[r.date] = (dailyMins[r.date]||0) + r.min; });
  
  const data = dates.map(function(dt, idx) { 
    let windowVals = []; 
    for(let i = 0; i < 7; i++) { 
  ...
  if(cvCanvas) {
    CH.dpCv = new Chart(cvCanvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          legend: { display: false },
          tooltip: {backgroundColor: 'rgba(61,61,92,.95)', callbacks: { label: function(c) { return 'CV: ' + c.parsed.y.toFixed(2); } }},
          datalabels: { display: false }
        },
        scales: {
...
function renderDpStacked(d, mos, subMap, color) {
  ...
  if(legendWrap) { 
    legendWrap.innerHTML = subList.map(function(sub, i) { 
      const isHL = H.dpSub === sub; 
      const isDim = H.dpSub && H.dpSub !== sub; 
      const c = SUB_PAL[i % SUB_PAL.length]; 
      return '<div style="display:flex; align-items:center; gap:4px; font-size:11px; cursor:pointer; transition:all 0.2s; ' + (isHL ?' font-weight:800; color:' + c + ';' : isDim ? 'opacity:0.4;' : 'color:#555;') + '" onclick="dpClickSub(\'' + sub + '\')"><div style="width:10px; height:10px; border-radius:3px; background:' + c + ';"></div>' + sub + '</div>'; 
    }).join(''); 
  }
  ...
  const datasets = subList.map(function(sub,i) { 
    const bgColors = mos.map(function(m) { 
    ...
    return { type: 'bar', label: sub, data: mos.map(function(m){ return mToH((moSubMap[m] && moSubMap[m][sub]) || 0); }), backgroundColor: bgColors, borderRadius: 2, stack: 's' }; 
  });
  ...
  CH.dpStackedBar=new Chart(dsBar.getContext('2d'),{
    data:{
      labels: mos.map(function(m){ return m.replace(/(\d{4})-(\d{2})/,'$1/$2'); }),
      datasets: datasets
    },
    options:{
      responsive:true, maintainAspectRatio:false,
      plugins:{
        ...
        datalabels:{
          ...
          formatter: function(val, ctx) { 
            ...
            if (H.dpSub === null) { 
              const dsTotals = ctx.chart.data.datasets.filter(function(ds){ return ds.label !== '주요일정'; }).map(function(ds){ return ds.data[ctx.dataIndex]; }); 
              let lastValIndex = -1; 
              for(let i = dsTotals.length-1; i >= 0; i--) { 
                if(dsTotals[i] > 0) { lastValIndex = i; break; } 
              } 
              return ctx.datasetIndex === lastValIndex && dsTotals.reduce(function(a,b){ return a+b; },0) > 0 ? formatNum(dsTotals.reduce(function(a,b){ return a+b; },0)) : ''; 
            } else return (ctx.dataset.label === H.dpSub && val > 0) ? formatNum(val) : ''; 
          },
          anchor: function(ctx) { return H.dpSub === null ? 'end' : 'center'; },
          align: function(ctx) { return H.dpSub === null ? 'top' : 'center'; },
          color: function(ctx) { return H.dpSub === null ? '#3d3d5c' : '#fff'; },
          font: function(ctx) { return { weight: H.dpSub === null ? 'normal' : '800', size: 11 }; }
        }
      },
      scales:{
        x:{stacked:true, grid:{display:false}, ticks:{font:{size:10}}},
        y:{stacked:true, grace: '45%', grid:{color:'#f0f1f5'}, ticks:{font:{size:10}, callback: function(v){ return formatNum(v)+'h'; }}},
...
function renderDpStackedHighlight(){
  ...
  if(!name)return;
  const d=filtered().filter(function(r){ return r.name===name; });
  const mos=getMos(d);
...
function renderPatternAnalysis(d, mos) {
  ...
  let allSwitches = []; 
  MEMBERS.forEach(function(mName) { 
    const md = filtered().filter(function(r) { return r.name === mName; }); 
    let sByMo = {}; let pr = null; 
    ...
    Object.values(sByMo).forEach(function(v) { allSwitches.push(v); }); 
  });
    ...
    CH.dpSwitchBar = new Chart(swCanvas.getContext('2d'), {
      ...
      options: {
        ...
        plugins: {
          ...
          datalabels: {
            display: function(ctx) { return ctx.dataset.label === '업무 전환 횟수' && (!G.dpActiveMonth || mos[ctx.dataIndex] === G.dpActiveMonth); },
            formatter: function(v) { return v + '회'; },
            color: '#555', anchor: 'end', align: 'top', font: { size: 10, weight: 'normal' }
          },
          tooltip: {
            backgroundColor:'rgba(61,61,92,.95)',
            filter: function(item) { return item.dataset.label === '전체 평균'; },
            callbacks:{ label: function(c) { return '팀원전체 평균 : ' + c.raw.toFixed(1) + '회'; } }
          }
        },
        scales: {
  ...
  const weeksSet = new Set(d.map(function(r){ return r.week; }));
  const weeks = Array.from(weeksSet).sort();
  
  const bubbleData = weeks.map(function(w) {
    const rWk = d.filter(function(r) { return r.week === w; }); 
    if(rWk.length === 0) return null;
    
    const totMin = rWk.reduce(function(s,x) { return s+x.min; }, 0); 
    const sMap = grpMin(rWk, 'sub');
    let hhiSum = 0; let shannon = 0; 
    
    Object.values(sMap).forEach(function(v) { 
      const p = v / totMin; hhiSum += p * p; 
      if(p > 0) shannon -= p * Math.log(p); 
    });
    ...
    CH.dpDiversityBubble = new Chart(dbCanvas.getContext('2d'), {
      ...
      data: {
        datasets: [{
          label: '주별 다양성',
          data: bubbleData,
          backgroundColor: function(ctx) { return ctx.raw ? ((G.dpActiveMonth && ctx.raw.moStr !== G.dpActiveMonth) ? hexRgba(ctx.raw.baseColor, 0.1) : hexRgba(ctx.raw.baseColor, 0.7)) : undefined; },
          borderColor: function(ctx) { return ctx.raw ? ((G.dpActiveMonth && ctx.raw.moStr !== G.dpActiveMonth) ? hexRgba(ctx.raw.baseColor, 0.2) : ctx.raw.baseColor) : undefined; }
        }]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
        plugins: {
          ...
          tooltip: {
            backgroundColor:'rgba(61,61,92,.95)',
            callbacks: {
              label: function(c) { return [c.raw.moStr + ' (' + c.raw.wk.split('-')[1] + ')', '다양성 지수(1-HHI): ' + c.raw.x.toFixed(2), 'Shannon: ' + c.raw.y.toFixed(2), '총 투입: ' + c.raw.time + 'h']; }
            }
          },
          datalabels: { display: false }
        },
  ...
  const workDatesSet = new Set(d.map(function(r){ return r.date; }));
  ...
  Object.keys(grpMin(d, 'sub')).forEach(function(s) {
    ...
    workDates.forEach(function(wd) {
      if(d.some(function(r) { return r.date === wd && r.sub === s; })) { 
...
function renderEvalTab(){
  ...
  drawOvertimeChart('evalOvertimeCardWrap', filtered().filter(function(r) { return r.name === G_evalMember; }));
  renderCorrelationChartInit();
}

function renderEvalDonut(hl){
  const name = G_evalMember; 
  const all = filtered().filter(function(r) { return r.name === name; }); 
  const subMap = grpMin(all,'sub'); 
  const totalMin = all.reduce(function(s,r) { return s+r.min; }, 0);
  ...
  if(top3wrap) {
    top3wrap.innerHTML = top.slice(0,3).map(function(item, i) {
      const sub = item[0];
      const min = item[1];
      return '<div class="eval-top3-box-card" style="background:' + ['#fef9ee','#f8fafc','#fdf6f0'][i] + ';border:1.5px solid ' + ['#fde68a','#e2e8f0','#fed7aa'][i] + '44;"><div style="font-size:18px;margin-bottom:4px;">' + ['🥇','🥈','🥉'][i] + '</div><div style="font-size:12px;font-weight:700;color:#3d3d5c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;width:100%;">' + sub + '</div><div style="font-size:14px;font-weight:900;color:' + (i===0?'#f59e0b':i===1?'#64748b':'#cd7c3b') + ';margin-top:2px;">' + formatHour(min) + 'h</div><div style="font-size:10px;color:#aaa;">' + (totalMin>0?Math.round(min/totalMin*100):0) + '% 비중</div></div>';
    }).join('');
  }
  ...
  CH.evalDonut = new Chart(edc.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        data: data, 
        backgroundColor: hl ? labels.map(function(l,i) { return l === hl ? colors[i] : colors[i] + '44'; }) : colors, 
        borderWidth: 2, 
        borderColor: '#fff', 
        hoverOffset: 8
      }]
...
function renderEvalTrendChart(hlLabel = null){
  ...
  const datasets = RADAR_LABELS.map(function(lbl, i) { 
    const isDimmed = hlLabel !== null && hlLabel !== lbl; 
    return {
      label: lbl, 
      data: qs.map(function(q) { const ev = EVAL_DATA.find(function(e) { return e.q === q && e.name === name; }); return ev ? ev.vals[i] : 0; }), 
  ...
  const scheduleDataArray = qs.map(function(q) { return SCHEDULE_DATA.filter(function(s){ return getQFromDate(s.date) === q; }).length > 0 ? 5.2 : null; }); 
  const scheduleEvtsArray = qs.map(function(q) { return SCHEDULE_DATA.filter(function(s){ return getQFromDate(s.date) === q; }); });
  ...
  if(legendEl) {
    legendEl.innerHTML = RADAR_LABELS.map(function(lbl, i) {
      return '<div class="legend-item" style="opacity:' + (hlLabel!==null&&hlLabel!==lbl?'0.35':'1') + '; font-weight:' + (hlLabel===lbl?'800':'normal') + ';" onclick="renderEvalTrendChart(\'' + (hlLabel===lbl?'':lbl) + '\')"><div class="legend-dot" style="background:' + RADAR_COLORS_ARR[i] + ';width:10px;height:10px;border-radius:3px;"></div>' + lbl + '</div>';
    }).join('');
  }
  ...
  CH.evalTrendChart = new Chart(etc.getContext('2d'), {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false }, 
        tooltip: { backgroundColor: 'rgba(61,61,92,.95)', callbacks: { label: function(c) { return c.dataset.label === '주요일정' ? c.dataset.customEvts[c.dataIndex].map(function(e){ return e.date + ': ' + e.title; }) : c.dataset.label + ': ' + c.parsed.y.toFixed(1) + '점'; } } }, 
        datalabels: { display: function(ctx){ return ctx.dataset.label !== '주요일정'; }, anchor: 'end', align: 'end', color: function(ctx){ return (hlLabel !== null && hlLabel !== ctx.dataset.label) ? 'transparent' : '#555'; }, font: { size: 9, weight: '700' }, formatter: function(v){ return v > 0 ? v.toFixed(1) : ''; } }
      }, 
      scales: {
        x: { grid: { display: false }, ticks: { font: { size: 11, weight: '700' }, color: '#3d3d5c' } }, 
        y: { min: 0, max: 5.5, grid: { color: '#eef5fb' }, ticks: { font: { size: 10 }, callback: function(v){ return v > 5 ? '' : v + '점'; } }, title: { display: true, text: '점수', font: { size: 10 } } }
      }, 
      onClick: function(e, els) { 
...
function renderEvalRadarDual(){
  ...
  const color = MC[name]; 
  const qs = filteredEvalQs(); 
  const allQsSet = new Set(EVAL_DATA.map(function(d){ return d.q; }));
  const allQs = Array.from(allQsSet);
  
  const yearAvgs = RADAR_LABELS.map(function(_,i) { 
    const vals = allQs.map(function(q) { 
      const ev = EVAL_DATA.find(function(e) { return e.q === q && e.name === name; }); 
      return ev ? ev.vals[i] : null; 
    }).filter(function(v) { return v !== null; }); 
    return vals.length > 0 ? parseFloat((vals.reduce(function(a,b){ return a+b; },0)/vals.length).toFixed(2)) : 0; 
  });
  ...
  if(latestQ) {
    const latestEv = EVAL_DATA.find(function(e) { return e.q === latestQ && e.name === name; });
    if(latestEv) qtrVals = latestEv.vals;
  }
  
  const yearAvgScore = yearAvgs.length ? parseFloat((yearAvgs.reduce(function(a,b){ return a+b; },0)/yearAvgs.length).toFixed(2)) : 0; 
  const qtrAvgScore = qtrVals.length ? parseFloat((qtrVals.reduce(function(a,b){ return a+b; },0)/qtrVals.length).toFixed(2)) : 0;
  ...
  const radarOpts = function(label, data, bc) {
    return {
      type: 'radar', 
      data: { labels: RADAR_LABELS, datasets: [{ label: label, data: data, backgroundColor: bc + '33', borderColor: bc, borderWidth: 2.5, pointBackgroundColor: bc, pointRadius: 4, pointHoverRadius: 6 }] }, 
      options: {
        layout: { padding: 16 }, 
        responsive: true, 
        maintainAspectRatio: false, 
        scales: { r: { min: 0, max: 5, ticks: { display: false, stepSize: 1 }, grid: { color: '#dbeafe' }, angleLines: { color: '#dbeafe' }, pointLabels: { font: { size: 10, weight: '700' }, color: '#3d3d5c', padding: 12 } } }, 
        plugins: {
          legend: { display: false }, 
          datalabels: { display: true, color: bc, font: { size: 10, weight: '800' }, formatter: function(v){ return v > 0 ? v.toFixed(1) : ''; }, align: 'end', anchor: 'end', offset: 3, textStrokeColor: '#fff', textStrokeWidth: 3 }, 
          tooltip: { backgroundColor: '#3d3d5c', callbacks: { label: function(c){ return c.label + ': ' + c.raw + '점'; } } }
        }
      }
    };
  };
...
function renderEvalTrendTable(){
  ...
  RADAR_LABELS.forEach(function(l, i) {
  ...
  qs.forEach(function(q, idx) {
    ...
    const prevEv = qs[idx+1] ? EVAL_DATA.find(function(e) { return e.q === qs[idx+1] && e.name === name; }) : null;
    
    html += '<tr class="' + (isLatest ? 'current-q' : '') + '"><td style="font-weight:700;color:' + (isLatest ? '#00458c' : '#555') + ';">';
    
    if(!ev){ 
      html += q + '</td>';
      RADAR_LABELS.forEach(function() { html += '<td>-</td>'; });
      html += '<td>-</td></tr>'; 
      return; 
    }
    
    const avg = (ev.vals.reduce(function(a,b){ return a+b; },0) / ev.vals.length); 
    const ac = avg >= 4 ? '#22c55e' : avg >= 3 ? '#00458c' : '#f97316';
    
    if(isLatest && prevEv) { 
      const diff = avg - (prevEv.vals.reduce(function(a,b){ return a+b; },0) / prevEv.vals.length); 
...
function renderEvalProfile(){
  ...
  const totalItemAvgs = RADAR_LABELS.map(function(lbl, i) { 
    let avgVal = 0;
    const vals = allQs.map(function(q) { 
      const ev = EVAL_DATA.find(function(e){ return e.q === q && e.name === name; }); 
      return ev ? ev.vals[i] : null; 
    ...
    if(vals.length > 0) {
      avgVal = parseFloat((vals.reduce(function(a,b){ return a+b; },0)/vals.length).toFixed(2));
    }
    return { lbl: lbl, avg: avgVal, idx: i }; 
  });
  
  const filteredItemAvgs = RADAR_LABELS.map(function(lbl, i) {
    let avgVal = 0;
    const vals = qs.map(function(q) { 
      const ev = EVAL_DATA.find(function(e){ return e.q === q && e.name === name; }); 
      return ev ? ev.vals[i] : null; 
    }).filter(function(v){ return v !== null; }); 
    if(vals.length > 0) {
  ...
  const getTrend = function(idx) { 
    if(qs.length < 2) return ''; 
    const curEv = EVAL_DATA.find(function(e) { return e.q === qs[qs.length-1] && e.name === name; }); 
    const prevEv = EVAL_DATA.find(function(e) { return e.q === qs[qs.length-2] && e.name === name; }); 
  ...
  if(card) {
    ...
    filteredItemAvgs.forEach(function(item, i) {
       const vColor = item.avg>=4.5 ? '#22c55e' : item.avg>=3.5 ? '#00458c' : item.avg>=2.5 ? '#f97316' : '#ef4444';
       html += '<div class="eval-score-item"><span class="eval-score-item-label">' + item.lbl + '</span><div class="eval-score-item-bar-wrap"><div class="eval-score-item-bar" style="width:' + Math.round(item.avg/5*100) + '%;background:' + RADAR_COLORS_ARR[i] + ';"></div></div><span class="eval-score-item-val" style="color:' + vColor + ';">' + item.avg.toFixed(1) + '</span></div>';
    });
    ...
    sorted.slice(0,3).forEach(function(item, rank) {
       const vColor = item.avg>=4 ? '#22c55e' : item.avg>=3 ? '#00458c' : '#f97316';
       html += '<div class="eval-top3-card" style="background:' + ['#fef9ee','#f8fafc','#fdf6f0'][rank] + ';border:1.5px solid ' + ['#f59e0b','#94a3b8','#cd7c3b'][rank] + '33;"><div class="eval-top3-medal">' + ['🥇','🥈','🥉'][rank] + '</div><div class="eval-top3-label">' + item.lbl + '</div><div class="eval-top3-score" style="color:' + vColor + ';">' + item.avg.toFixed(1) + '</div>' + getTrend(item.idx) + '</div>';
    });
  ...
  let qtrVals = [0,0,0,0,0];
  if(qs.length > 0) {
    const latestEv = EVAL_DATA.find(function(e) { return e.q === qs[qs.length-1] && e.name === name; });
    if(latestEv) qtrVals = latestEv.vals;
  }
...
function renderCorrelationChartInit() {
  ...
  if(tabsWrap) { 
    let html = '';
    RADAR_LABELS.forEach(function(lbl) {
...
function renderCorrelationChart() {
  ...
  const topSubs = Object.entries(overallSubMap).sort(function(a,b){ return b[1]-a[1]; }).slice(0, 7).map(function(x){ return x[0]; });
  
  const qMap = {}; 
  qs.forEach(function(q){ qMap[q] = {}; });
  ...
  const datasets = displaySubs.map(function(sub, i) { 
    return { 
      type: 'bar', 
      label: sub, 
      data: qs.map(function(q){ return mToH(qMap[q][sub] || 0); }), 
  ...
  const scoreData = qs.map(function(q) { 
    const ev = EVAL_DATA.find(function(e) { return e.q === q && e.name === name; }); 
      ...
      options: {
          ...
          plugins: {
              ...
              tooltip: { 
                backgroundColor: 'rgba(61,61,92,.95)', 
                callbacks: { 
                  label: function(c) { return c.dataset.type === 'line' ? '⭐ ' + c.dataset.label + ': ' + c.parsed.y.toFixed(1) + '점' : c.dataset.label + ': ' + formatNum(c.parsed.y) + 'h'; } 
                } 
              }, 
              datalabels: { 
