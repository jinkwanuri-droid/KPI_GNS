...
function renderProjectProgress() {
    var wrap = document.getElementById('projectProgressWrap');
    if(!wrap) return;
    var milestones = [
        { key: '설계공모', date: '2024-03-01' }, { key: '착수보고회', date: '2024-05-15' },
        { key: '계획설계 적정성', date: '2024-09-30' }, { key: '건축교통통합심의', date: '2024-11-30' },
        { key: '중간설계 적정성', date: '2025-01-31' }, { key: '건축허가', date: '2025-02-20' },
        { key: '프로젝트 종료', date: '2025-12-31' }
    ];
    if (SCHEDULE_DATA && SCHEDULE_DATA.length > 0) {
        SCHEDULE_DATA.forEach(function(s) {
            var t = s.title || s.name || s['일정'] || s['내용'] || '';
            var d = s.date ? s.date.slice(0,10) : '';
            if(!d) return;
            if(t.includes('설계공모')) milestones[0].date = d;
            if(t.includes('착수보고')) milestones[1].date = d;
            if(t.includes('계획설계 적정성')) milestones[2].date = d;
            if(t.includes('통합심의') || t.includes('건축교통')) milestones[3].date = d;
            if(t.includes('중간설계 적정성')) milestones[4].date = d;
            if(t.includes('건축허가')) milestones[5].date = d;
            if(t.includes('종료')) milestones[6].date = d;
        });
    }
    milestones.sort(function(a,b){ return new Date(a.date) - new Date(b.date); });
    var st = new Date(milestones[0].date).getTime(), ed = new Date(milestones[milestones.length-1].date).getTime();
    var now = new Date().getTime(), total = ed - st, current = now - st, pct = 0;
    if(total > 0) { pct = Math.max(0, Math.min(100, (current / total) * 100)); }
    
    var dotHtml = milestones.map(function(m) {
        var mTime = new Date(m.date).getTime();
        var mPct = total > 0 ? Math.max(0, Math.min(100, ((mTime - st) / total) * 100)) : 0;
        var dotColor = mPct <= pct ? '#1e3a8a' : '#fff';
        return '<div class="pj-prog-dot" style="left: '+mPct+'%; background: '+dotColor+';">' +
               '<div class="pj-prog-tooltip">' + m.key + '<br><span style="color:#93c5fd;font-size:7.5px;">(' + m.date + ') [' + mPct.toFixed(1) + '%]</span></div>' +
               '</div>';
    }).join('');
    
    var html = '<div class="pj-progress-wrap">' +
        '<div class="pj-prog-header">' +
            '<div>' +
                '<div class="pj-prog-title" style="font-size:14px; font-weight:700; color:#475569; text-transform:none; letter-spacing:0;">Project Progress</div>' +
                '<div class="pj-prog-pct">' + pct.toFixed(1) + '% <span style="font-size:16px;font-weight:700;color:#64748b;">to complete</span></div>' +
            '</div>' +
            '<div style="font-size:11px; color:#94a3b8; font-weight:700; display:flex; align-items:center; gap:4px; margin-bottom:4px;">' +
            ...
            '</div>' +
        '</div>' +
        '<div class="pj-prog-bar-bg"><div class="pj-prog-bar-fill" style="width: '+pct+'%;"></div>' + dotHtml + '</div>' +
    '</div>';
    wrap.innerHTML = html;
}

function renderOvProjectRatio(all){
    dC('ovPjRatioBar');var m=0,o=0,c=0;
    all.forEach(function(r){if(r.project==='경남 서부의료원')m+=r.min;else if(r.project==='타 프로젝트')o+=r.min;else c+=r.min;});
    var t=m+o+c,it=[{k:'경남 서부의료원',c:'#00428E'},{k:'타 프로젝트',c:'#3b82f6'},{k:'공통업무',c:'#cbd5e1'}];
    document.getElementById('ovPjRatioLegendWrap').innerHTML=it.map(function(i){return '<div class="legend-item '+(hlPjRatio!==null&&hlPjRatio!==i.k?'dimmed':'')+'" onclick="ovTogglePjR(\''+i.k+'\')"><div class="legend-dot" style="background:'+i.c+'"></div><span style="font-size:11px;">'+i.k.replace('경남 ','')+'</span></div>';}).join('');
    CH.ovPjRatioBar=new Chart(document.getElementById('ovPjRatioBar').getContext('2d'),{
        type:'bar', data:{ labels:[''], datasets:[
                {label:'경남 서부의료원',data:[mToH(m)],backgroundColor:(hlPjRatio===null||hlPjRatio==='경남 서부의료원')?'#00428E':'#e2e8f0',barThickness:60,borderRadius:{topLeft:10,bottomLeft:10}},
                {label:'타 프로젝트',data:[mToH(o)],backgroundColor:(hlPjRatio===null||hlPjRatio==='타 프로젝트')?'#3b82f6':'#e2e8f0',barThickness:60},
                {label:'공통업무',data:[mToH(c)],backgroundColor:(hlPjRatio===null||hlPjRatio==='공통업무')?'#cbd5e1':'#e2e8f0',barThickness:60,borderRadius:{topRight:10,bottomRight:10}}
            ] },
        options:{
            indexAxis:'y',responsive:true,maintainAspectRatio:false,interaction: { mode: 'point', intersect: true },
            plugins:{
                legend:{display:false},
                datalabels:{ display:function(cx){return cx.datasetIndex===2?'#334155':'#fff';}, font:{weight:'bold',size:12},anchor:'center',align:'center',textAlign:'center', formatter:function(v,cx){if(v<=0)return'';return fmt(v)+'h\n('+((v/mToH(t)*100).toFixed(1))+'%)';} },
                tooltip: { callbacks: { label: function(cx) { return cx.dataset.label + ': ' + fmt(cx.raw) + 'h'; } } }
            },
...
function renderCostTrendChart(mos) {
    ...
    if (endMonth.startsWith('2026')) { endMonth = '2026-11'; }
    var fullMonths = getMonthsBetween(startMonth, endMonth);
    var allMosMap = {};
        ...
        options: {
            ...
            plugins: {
                ...
                datalabels: {
                    ...
                    offset: 6, color: function(cx) { return cx.dataset.borderColor; }, font: { size: 10, weight: 'bold' }, formatter: function(v) { return Math.round(v); }
                },
                tooltip: {
            ...
            scales: { x: { grid: { display: false }, ticks: { maxTicksLimit: 12, maxRotation: 0 } }, y: { grid: { color: 'rgba(226,232,240,0.5)' }, min: 0, max: scheduleTopY, ticks: { stepSize: 10, callback: function(v) { return v.toLocaleString('ko-KR') + '천만'; } } } }
        }
    });
...
function renderDashStage(all){
    dC('stageChart');var cm=grp(all,'cat'),t=Object.values(cm).reduce(function(a,b){return a+b;},0);if(!t)return;
    ...
    CH.stageChart=new Chart(document.getElementById('stageChart').getContext('2d'),{
        type:'doughnut',
        data:{labels:CAT_ORDER,datasets:[{data:CAT_ORDER.map(function(c){return mToH(cm[c]||0);}),backgroundColor:CAT_ORDER.map(function(c,i){return (hlStage===null||hlStage===c)?CAT_COLORS[i]:'#e2e8f0';}),borderWidth:2,borderColor:'#fff'}]},
        options:{
            responsive:true,maintainAspectRatio:false,cutout:'65%',layout:{padding:{top:25,bottom:25,left:35,right:35}}, interaction: { mode: 'nearest', intersect: true },
            plugins:{ legend:{display:false}, datalabels:{display:true,color:'#1e293b',font:{weight:'bold',size:10},anchor:'end',align:'end',textAlign:'center',formatter:function(v,cx){var p=(v/mToH(t)*100);if(p<5)return'';return cx.chart.data.labels[cx.dataIndex]+'\n'+fmt(v)+'h ('+p.toFixed(1)+'%)';}} },
            onClick:function(e,els){ if(els.length) dTogStage(CH.stageChart.data.labels[els[0].index]); else { hlStage=null; var p=filtered().filter(function(r){return r.project==='경남 서부의료원';}); renderDashStage(p); renderDashCompare(p); } }
        }
    });
...
function renderDashCompare(all){
    dC('compareChart');if(!all.length)return;
    var ds=MEMBERS.map(function(n){
        return { label: n, data: CAT_ORDER.map(function(c){ return mToH(all.filter(function(r){return r.name===n&&r.cat===c;}).reduce(function(s,x){return s+x.min;},0)); }), backgroundColor: CAT_ORDER.map(function(c){ if(hlStage!==null && hlStage!==c) return '#e2e8f0'; if(hlTimeline!==null && hlTimeline!==n) return '#e2e8f0'; return MC[n]; }), borderRadius:4 };
    });
    ...
    CH.compareChart=new Chart(document.getElementById('compareChart').getContext('2d'),{
        ...
        options:{ responsive:true,maintainAspectRatio:false,layout:{padding:{top:15}}, interaction: { mode: 'nearest', intersect: true }, plugins:{ legend:{display:false}, datalabels:{ display:function(cx){ var stg = CAT_ORDER[cx.dataIndex]; var mem = cx.dataset.label; if(hlStage!==null && hlStage!==stg) return false; if(hlTimeline!==null && hlTimeline!==mem) return false; return cx.dataset.data[cx.dataIndex] > 0; }, color:'#64748b',font:{size:10,weight:'bold'},anchor:'end',align:'end', formatter:function(v){return v>0?v.toFixed(1):'';} } }, scales:{x:{grid:{display:false}},y:{grid:{color:'rgba(226,232,240,0.5)'},grace:'10%'}}, onClick:function(e,els){ if(els.length) { dTogStage(CAT_ORDER[els[0].index]); } else { dTogStage(null); } } }
    });
}

function renderDashDonut(all){
    dC('donutChart');var sm=grp(all,'sub'),t=Object.values(sm).reduce(function(a,b){return a+b;},0),ta=Object.entries(sm).sort(function(a,b){return b[1]-a[1];});
    ...
    var l=ta.slice(0,7).map(function(e){return String(e[0]);}),d=ta.slice(0,7).map(function(e){return mToH(e[1]);});
    CH.donutChart=new Chart(document.getElementById('donutChart').getContext('2d'),{
        type:'doughnut', data:{labels:l,datasets:[{data:d,backgroundColor:l.map(function(lx,i){return (hlDonutWork===null||hlDonutWork===lx)?SUB_PAL[i]:'#e2e8f0';}),borderWidth:2,borderColor:'#fff'}]},
        options:{ responsive:true,maintainAspectRatio:false,cutout:'60%',layout:{padding:{top:40,bottom:40,left:50,right:50}}, interaction: { mode: 'nearest', intersect: true }, plugins:{ legend:{display:false}, datalabels:{display:true,color:'#1e293b',font:{size:10,weight:'800'},textAlign:'center',formatter:function(v,cx){var p=(v/mToH(t)*100);if(p<5)return'';return cx.chart.data.labels[cx.dataIndex]+'\n'+fmt(v)+'h ('+p.toFixed(1)+'%)';},anchor:'end',align:'end'} }, onClick:function(e,els){ if(els.length) dTogDonut(CH.donutChart.data.labels[els[0].index]); else { hlDonutWork=null; renderDashDonut(filtered().filter(function(r){return r.project==='경남 서부의료원';})); } } }
    });
}

function renderDashTimeline(all,mos){
    dC('timelineChart');if(!mos.length)return;
    var ds=MEMBERS.map(function(n){var mm={};all.filter(function(r){return r.name===n;}).forEach(function(r){mm[r.date.slice(0,7)]=(mm[r.date.slice(0,7)]||0)+r.min;});var c=MC[n],ih=hlTimeline===null||hlTimeline===n;return {label:n,data:mos.map(function(m){return mToH(mm[m]||0);}),borderColor:ih?c:'#e2e8f0',backgroundColor:ih?hRgba(c,0.15):'transparent',borderWidth:2,tension:0.3,fill:true,pointRadius:2,pointHoverRadius:5};});
    document.getElementById('timelineLegendWrap').innerHTML=MEMBERS.map(function(n){return '<div class="legend-item '+(hlTimeline!==null&&hlTimeline!==n?'dimmed':'')+'" onclick="dTogTimeline(\''+n+'\')"><div class="legend-dot" style="background:'+MC[n]+'"></div>'+n+'</div>';}).join('');
...
function renderDashOvertimeBars(all){
    dC('dashOvertimeBarChart');var dm={};all.forEach(function(r){var k=r.name+'|'+r.date;dm[k]=(dm[k]||0)+r.min;});var c=[0,0,0,0];Object.values(dm).forEach(function(m){if(m>=540&&m<600)c[0]++;else if(m>=600&&m<660)c[1]++;else if(m>=660&&m<720)c[2]++;else if(m>=720)c[3]++;});
    CH.dashOvertimeBarChart=new Chart(document.getElementById('dashOvertimeBarChart').getContext('2d'),{
        type:'bar', data:{labels:OVERTIME_RANGES,datasets:[{data:c,backgroundColor:c.map(function(_,i){return (hlMonthlyOt===null||hlMonthlyOt===OVERTIME_RANGES[i])?OT_COLORS[i]:'#e2e8f0';}),borderRadius:4}]},
        options:{ indexAxis:'y',responsive:true,maintainAspectRatio:false,layout:{padding:{right:30}}, interaction: { mode: 'nearest', intersect: true }, plugins:{legend:{display:false},datalabels:{display:true,color:'#1e293b',font:{weight:'bold'},anchor:'end',align:'right',formatter:function(v){return v>0?v+'일':'';}}}, scales:{x:{display:false,grace:'15%'},y:{grid:{display:false},ticks:{font:{size:11,weight:'bold'}}}}, onClick:function(e,els){ if(els.length) dTogMonthlyOt(OVERTIME_RANGES[els[0].index]); else{hlMonthlyOt=null;var p=filtered().filter(function(r){return r.project==='경남 서부의료원';});renderDashMonthlyOt(p,getMos(p));renderDashOvertimeBars(p);} } }
    });
}

function renderDashMonthlyOt(all,mos){
    dC('dashMonthlyOvertimeChart');
    var bd=['9~10h','10~11h','11~12h','12h 이상'], bda=[[],[],[],[]], pd={};
    all.forEach(function(r){var k=r.name+'|'+r.date; pd[k]=(pd[k]||0)+r.min;});
    mos.forEach(function(m){ var c=[0,0,0,0]; Object.keys(pd).forEach(function(k){ if(k.split('|')[1].startsWith(m)){ var mn=pd[k]; if(mn>=540&&mn<600)c[0]++; else if(mn>=600&&mn<660)c[1]++; else if(mn>=660&&mn<720)c[2]++; else if(mn>=720)c[3]++; } }); c.forEach(function(v,i){bda[i].push(v);}); });
    document.getElementById('dashMonthlyOtLegendWrap').innerHTML=bd.map(function(b,i){return '<div class="legend-item '+(hlMonthlyOt!==null&&hlMonthlyOt!==b?'dimmed':'')+'" onclick="dTogMonthlyOt(\''+b+'\')"><div class="legend-dot" style="background:'+OT_COLORS[i]+'"></div><span style="font-size:12px;">'+b+'</span></div>';}).join('');
    CH.dashMonthlyOvertimeChart=new Chart(document.getElementById('dashMonthlyOvertimeChart').getContext('2d'),{
        type:'bar', data:{labels:mos,datasets:bd.map(function(b,i){return {label:b,data:bda[i],backgroundColor:(hlMonthlyOt===null||hlMonthlyOt===b)?OT_COLORS[i]:'#e2e8f0',stack:'S0',borderRadius:2};})},
        options:{ responsive:true,maintainAspectRatio:false,layout:{padding:{top:20}}, interaction: { mode: 'nearest', intersect: true }, plugins:{ legend:{display:false}, datalabels:{ display: function(cx){ if(hlMonthlyOt === null) { var maxIdx = -1; for(var i = 3; i >= 0; i--) { if(cx.chart.data.datasets[i].data[cx.dataIndex] > 0) { maxIdx = i; break; } } return cx.datasetIndex === maxIdx; } else { return cx.dataset.label === hlMonthlyOt && cx.dataset.data[cx.dataIndex] > 0; } }, formatter: function(v, cx) { if(hlMonthlyOt === null) { var sum = 0; cx.chart.data.datasets.forEach(function(ds){ sum += ds.data[cx.dataIndex] || 0; }); return sum > 0 ? sum + '일' : ''; } else { return v > 0 ? v + '일' : ''; } }, color: function(cx) { return hlMonthlyOt === null ? '#1e293b' : '#fff'; }, font: { size: 10, weight: 'bold' }, anchor: function(cx) { return hlMonthlyOt === null ? 'end' : 'center'; }, align: function(cx) { return hlMonthlyOt === null ? 'bottom' : 'center'; } } }, scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,display:false,grace:'20%'}}, onClick:function(e,els){ if(els.length) dTogMonthlyOt(CH.dashMonthlyOvertimeChart.data.datasets[els[0].datasetIndex].label); else { hlMonthlyOt=null; var p=filtered().filter(function(r){return r.project==='경남 서부의료원';}); renderDashMonthlyOt(p,getMos(p)); renderDashOvertimeBars(p); } } }
    });
}

function renderDashOvertimeDonut(all){
    dC('dashOvertimeDonutChart9'); dC('dashOvertimeDonutChart12');
    var dm=[],ds=[];
    MEMBERS.forEach(function(n){ var pd={}; all.filter(function(r){return r.name===n;}).forEach(function(r){pd[r.date]=(pd[r.date]||0)+r.min;}); var o9=0,o12=0; Object.values(pd).forEach(function(m){if(m>=540)o9++;if(m>=720)o12++;}); if(o9>0)dm.push({name:n,days:o9}); if(o12>0)ds.push({name:n,days:o12}); });
    document.getElementById('dashOvertimeDonutLegendWrap').innerHTML=MEMBERS.map(function(n){ return '<div class="legend-item '+(hlOvertimeDonut!==null&&hlOvertimeDonut!==n?'dimmed':'')+'" onclick="dTogOtDonut(\''+n+'\')"><div class="legend-dot" style="background:'+MC[n]+'"></div>'+n+'</div>'; }).join('');
    function dd(id,da){
        if(!da.length)return;
        CH[id]=new Chart(document.getElementById(id).getContext('2d'),{ type:'doughnut', data:{ labels:da.map(function(d){return d.name;}), datasets:[{ data:da.map(function(d){return d.days;}), backgroundColor:da.map(function(d){return (hlOvertimeDonut===null||hlOvertimeDonut===d.name)?MC[d.name]:'#e2e8f0';}), borderWidth:2,borderColor:'#fff' }] }, options:{ responsive:true, maintainAspectRatio:false, cutout:'65%', layout:{padding:{top:15,bottom:15,left:20,right:20}}, plugins:{ legend:{display:false}, datalabels:{ display:true,color:'#1e293b',font:{weight:'bold',size:10}, formatter:function(v,cx){ var s=cx.dataset.data.reduce(function(a,b){return a+b;},0),p=s>0?Math.round(v/s*100):0; return v>0?cx.chart.data.labels[cx.dataIndex]+'\n'+v+'일 ('+p+'%)':''; }, anchor:'end',align:'end' } }, onClick:function(e,els){ if(els.length) dTogOtDonut(da[els[0].index].name); else dTogOtDonut(null); } } });
    }
...
function renderWorkingTab(nm){
    ...
    var catStr = Object.entries(grp(d,'cat')).sort((a,b)=>b[1]-a[1]).map(x => x[0]+'('+(tt>0 ? (x[1]/tt*100).toFixed(0) : 0)+'%)').join(', ');
    window.currentWorkingAiData = { m: nm, tt: tt, apm: apm, o9: o9, o12: o12, tk: tk, as: as, ah: ah, ac: ac, cat: catStr };
    document.getElementById('wpAiCommentBox').innerHTML = '우측 상단의 <b>분석 요청하기</b> 버튼을 눌러 팀원의 근무 패턴과 설계 업무 분야 변화에 대한 10년 차 팀장의 심층 피드백을 받아보세요.';
...
function renderWpHeroDonut(d,t,c){
    dC('wpHeroDonut'); var sm=grp(d,'sub'),tl=Object.entries(sm).sort(function(a,b){return b[1]-a[1];}).slice(0,5);
    if(!tl.length){document.getElementById('wpTop3List').innerHTML='<div style="font-size:12px;color:#94a3b8;">데이터 없음</div>';return;}
    document.getElementById('wpTop3List').innerHTML=tl.map(function(e,i){ var k=String(e[0]),v=e[1],p=t>0?(v/t*100).toFixed(1):0,id=hlHeroDonut!==null&&hlHeroDonut!==k; return '<div class="wp-top5-item '+(id?'dimmed':'')+'" onclick="dTogHeroDonut(\''+k.replace(/'/g,"\\'")+'\')"><div class="wp-top5-dot" style="background:'+SUB_PAL[i]+';"></div><span class="wp-top5-name">'+k+'</span><span class="wp-top5-pct">'+p+'%</span></div>'; }).join('');
    var ae=Object.entries(sm).sort(function(a,b){return b[1]-a[1];}),lb=[],da=[],bc=[],om=0;
    ae.forEach(function(e,i){ if(i<5){ lb.push(String(e[0]));da.push(mToH(e[1])); bc.push(hlHeroDonut===null||hlHeroDonut===String(e[0])?SUB_PAL[i]:'#e2e8f0'); }else{om+=e[1];} });
    if(om>0){ lb.push('기타');da.push(mToH(om)); bc.push(hlHeroDonut===null||hlHeroDonut==='기타'?'#cbd5e1':'#e2e8f0'); }
    CH.wpHeroDonut=new Chart(document.getElementById('wpHeroDonut').getContext('2d'),{
        type:'doughnut', data:{labels:lb,datasets:[{data:da,backgroundColor:bc,borderWidth:2,borderColor:'rgba(255,255,255,0.8)'}]},
        options:{ responsive:true,maintainAspectRatio:false,cutout:'50%', interaction: { mode: 'nearest' }, plugins:{ legend:{display:false},datalabels:{display:false}, tooltip:{callbacks:{label:function(cx){var p=(cx.raw/mToH(Math.max(t,0.01))*100).toFixed(1);return cx.label+': '+cx.raw+'h ('+p+'%)';}}} }, onClick:function(e,els){if(els.length)dTogHeroDonut(lb[els[0].index]);else dTogHeroDonut(null);} }
    });
}

function renderWpMetricsBadges(d){
    var da=Array.from(new Set(d.map(function(r){return r.date;}))).sort(),as=0,ah=0,ac=0,cn=0;
    da.forEach(function(dt){
        var dy=d.filter(function(r){return r.date===dt;});if(!dy.length)return;
        var tt=dy.reduce(function(s,r){return s+r.min;},0),sm=grp(dy,'sub'),hi=0,sn=0,ky=Object.keys(sm),N=ky.length;
        ky.forEach(function(k){var p=sm[k]/tt;hi+=p*p;if(p>0)sn-=p*Math.log(p);});if(N>1)sn=sn/Math.log(N);else sn=0;
        var wv=[];for(var i=0;i<7;i++){var pd=new Date(new Date(dt).getTime()-i*86400000).toISOString().slice(0,10);wv.push(d.filter(function(r){return r.date===pd;}).reduce(function(a,b){return a+b.min;},0));}
        var m=wv.reduce(function(a,b){return a+b;},0)/7,cv=m===0?0:Math.sqrt(wv.reduce(function(a,b){return a+Math.pow(b-m,2);},0)/7)/m;
        as+=sn;ah+=hi;ac+=cv;cn++;
    });
    if(cn>0){as/=cn;ah/=cn;ac/=cn;}
    var se=document.getElementById('wmShan');se.textContent=as.toFixed(2);se.parentElement.className='wp-metric-badge '+(as<0.5?'good':as<0.8?'warn':'danger');
    var he=document.getElementById('wmHhi');he.textContent=ah.toFixed(3);he.parentElement.className='wp-metric-badge '+(ah<0.25?'good':ah<0.4?'warn':'danger');
    var ce=document.getElementById('wmCv');ce.textContent=ac.toFixed(2);ce.parentElement.className='wp-metric-badge '+(ac<0.5?'good':ac<1.0?'warn':'danger');
}
...
function renderWpStackedBar(d,mos){
    ...
    CH.wpStackedBar=new Chart(document.getElementById('wpStackedBar').getContext('2d'),{
        data:{ labels:mos, datasets:sl.map(function(s,i){ return { type:'bar', label:s, data:mos.map(function(m){return mToH((mm[m]||{})[s]||0);}), backgroundColor:(hlWpSub===null||hlWpSub===s)?SUB_PAL[i]:'#e2e8f0', stack:'s' }; }) },
        options:{ responsive:true, maintainAspectRatio:false, layout:{padding:{top:25}}, interaction: { mode: 'nearest', intersect: true }, plugins:{ legend:{display:false}, tooltip: { mode: 'nearest', intersect: true }, datalabels: { display: function(cx) { if (hlWpSub === null) { var maxIdx = -1; for(var i = cx.chart.data.datasets.length - 1; i >= 0; i--) { if(cx.chart.data.datasets[i].data[cx.dataIndex] > 0) { maxIdx = i; break; } } return cx.datasetIndex === maxIdx; } else { return cx.dataset.label === hlWpSub && cx.dataset.data[cx.dataIndex] > 0; } }, formatter: function(v, cx) { if (hlWpSub === null) { var sum = 0; cx.chart.data.datasets.forEach(function(ds){ sum += ds.data[cx.dataIndex] || 0; }); return sum > 0 ? fmt(sum) + 'h' : ''; } else { return v > 0 ? fmt(v) + 'h' : ''; } }, anchor: 'end', align: 'end', color: '#64748b', font: {size: 10, weight: 'bold'} } }, scales:{ x:{stacked:true,grid:{display:false}}, y:{stacked:true,grid:{color:'rgba(226,232,240,0.5)'}, grace:'15%'} }, onClick:function(e,els){if(els.length)dTogWpSub(CH.wpStackedBar.data.datasets[els[0].datasetIndex].label);else dTogWpSub(null);} }
    });
}

function renderWpDonutSub(d,t){
    dC('wpDonutSub'); var sm=grp(d,'sub'),ta=Object.entries(sm).sort(function(a,b){return b[1]-a[1];});
    ...
    CH.wpDonutSub=new Chart(document.getElementById('wpDonutSub').getContext('2d'),{
        type:'doughnut', data:{labels:ta.slice(0,7).map(function(e){return String(e[0]);}),datasets:[{data:ta.slice(0,7).map(function(e){return mToH(e[1]);}),backgroundColor:ta.slice(0,7).map(function(e,i){return (hlWpSub===null||hlWpSub===String(e[0]))?SUB_PAL[i]:'#e2e8f0';}),borderWidth:2,borderColor:'#fff'}]},
        options:{ responsive:true,maintainAspectRatio:false,cutout:'50%',layout:{padding:{top:20,bottom:20,left:20,right:20}}, interaction: { mode: 'nearest', intersect: true }, plugins:{legend:{display:false},datalabels:{display:function(cx){return (cx.dataset.data[cx.dataIndex]/mToH(t)*100)>=5;},color:'#1e293b',font:{size:10,weight:'bold'},anchor:'end',align:'end',textAlign:'center',formatter:function(v,cx){var p=(v/mToH(t)*100).toFixed(0);return ta[cx.dataIndex][0]+'\n'+p+'%';}}}, onClick:function(e,els){if(els.length)dTogWpSub(CH.wpDonutSub.data.labels[els[0].index]);else dTogWpSub(null);} }
    });
}

function renderWpDonutCat(d,t){
    dC('wpDonutCat');var cm=grp(d,'cat'),ca=CAT_ORDER.filter(function(c){return cm[c]>0;});
    ...
    CH.wpDonutCat=new Chart(document.getElementById('wpDonutCat').getContext('2d'),{
        type:'doughnut', data:{labels:ca,datasets:[{data:ca.map(function(c){return mToH(cm[c]);}),backgroundColor:ca.map(function(c){return CAT_COLORS[CAT_ORDER.indexOf(c)];}),borderWidth:2,borderColor:'#fff'}]},
        options:{ responsive:true,maintainAspectRatio:false,cutout:'60%',layout:{padding:{top:30,bottom:30,left:40,right:40}}, interaction: { mode: 'nearest', intersect: true }, plugins:{legend:{display:false},datalabels:{display:function(cx){return (cx.dataset.data[cx.dataIndex]/mToH(t)*100)>=5;},color:'#1e293b',font:{size:11,weight:'bold'},anchor:'end',align:'end',textAlign:'center',formatter:function(v,cx){var p=(v/mToH(t)*100).toFixed(0);return ca[cx.dataIndex]+'\n'+p+'%';}}} }
    });
}

function renderWpSwitchBar(d,mos,col){
    dC('wpSwitchBar');var sw={},pv=null;d.forEach(function(r){var m=r.date.slice(0,7);if(!sw[m])sw[m]=0;if(pv!==null&&pv!==r.sub)sw[m]++;pv=r.sub;});
    CH.wpSwitchBar=new Chart(document.getElementById('wpSwitchBar').getContext('2d'),{type:'bar',data:{labels:mos,datasets:[{data:mos.map(function(m){return sw[m]||0;}),backgroundColor:col,borderRadius:6}]},options:{responsive:true,maintainAspectRatio:false,layout:{padding:{top:15}},plugins:{legend:{display:false},datalabels:{display:true,color:col,font:{weight:'bold',size:10},anchor:'end',align:'end',formatter:function(v){return v>0?v:'';}}},scales:{x:{grid:{display:false}},y:{beginAtZero:true,grid:{color:'rgba(226,232,240,0.5)'}}}}});
}

function renderWpFocusBar(d,mos){
    dC('wpFocusBar');
    var fk=['도면','설계','검토','계획','보고서','작성','분석'],mf={},mt={};
    mos.forEach(function(m){mf[m]=0;mt[m]=0;});
    d.forEach(function(r){ var m=r.date.slice(0,7); if(mt[m]===undefined)return; var f=fk.some(function(k){return r.sub.includes(k)||r.cat.includes(k);}); if(f)mf[m]+=r.min; mt[m]+=r.min; });
    CH.wpFocusBar=new Chart(document.getElementById('wpFocusBar').getContext('2d'),{
        type:'line', data:{ labels:mos, datasets:[ { label:'집중업무 비율(%)', data:mos.map(function(m){return mt[m]>0?(mf[m]/mt[m]*100):0;}), borderColor:'#00428E', backgroundColor:hRgba('#00428E',0.1), borderWidth:2, fill:true, tension:0.3, pointRadius:4, pointBackgroundColor:'#00428E' } ] },
        options:{ responsive:true, maintainAspectRatio:false, layout:{padding:{top:25}}, plugins:{ legend:{display:false}, datalabels:{display:true, formatter:function(v){return v.toFixed(1)+'%';}, align:'top', font:{size:10, weight:'bold'}, color:'#00428E'} }, scales:{ x:{grid:{display:false}}, y:{min: 20, max: 110, grid:{color:'rgba(226,232,240,0.5)'}, ticks:{callback:function(v){return v > 100 ? '' : v+'%';}}} } }
    });
}

function renderWpOvertimeDetail(d){
    dC('wpOtBar'); var dm={}; d.forEach(function(r){dm[r.date]=(dm[r.date]||0)+r.min;});
    var od=Object.entries(dm).filter(function(e){return e[1]>=540;}).sort(function(a,b){return a[0].localeCompare(b[0]);});
    var mo=Array.from(new Set(od.map(function(e){return e[0].slice(0,7);}))).sort();
    var ct={'9~10h':[],'10~11h':[],'11~12h':[],'12h 이상':[]};
    mo.forEach(function(m){ var c1=0,c2=0,c3=0,c4=0; od.forEach(function(e){ if(!e[0].startsWith(m))return; var n=e[1]; if(n<600)c1++; else if(n<660)c2++; else if(n<720)c3++; else c4++; }); ct['9~10h'].push(c1); ct['10~11h'].push(c2); ct['11~12h'].push(c3); ct['12h 이상'].push(c4); });
    var bd = ['9~10h','10~11h','11~12h','12h 이상'];
    document.getElementById('wpOtLegendWrap').innerHTML = bd.map(function(b, i) { return '<div class="legend-item ' + (hlWpOt!==null && hlWpOt!==b ? 'dimmed' : '') + '" onclick="dTogWpOt(\''+b+'\')"><div class="legend-dot" style="background:'+OT_COLORS[i]+'"></div><span style="font-size:11px;">'+b+'</span></div>'; }).join('');
    CH.wpOtBar=new Chart(document.getElementById('wpOtBar').getContext('2d'),{
        ...
        options:{ responsive:true, maintainAspectRatio:false, layout:{padding:{top:25}}, interaction: { mode: 'nearest', intersect: true }, plugins:{ legend:{display:false}, datalabels:{ display: function(cx){ return cx.dataset.data[cx.dataIndex] > 0 && (hlWpOt===null || hlWpOt===cx.dataset.label); }, color: '#fff', font: {size:10, weight:'bold'}, formatter: function(v){ return v; } } }, scales:{ x:{stacked:true, grid:{display:false}, ticks:{font:{size:11}}}, y:{stacked:true, display:false, grace:'10%'} }, onClick: function(e,els){ if(els.length) dTogWpOt(CH.wpOtBar.data.datasets[els[0].datasetIndex].label); else dTogWpOt(null); } }
    });
}

function renderWpGini(d,c){
    dC('wpGiniChart');var dm={};d.forEach(function(r){dm[r.date]=(dm[r.date]||0)+r.min;});var vl=Object.values(dm).sort(function(a,b){return a-b;}),n=vl.length,gn=0;if(n>1){var sm=vl.reduce(function(a,b){return a+b;},0),cs=0;vl.forEach(function(v){cs+=v;gn+=cs;});gn=(2*gn/(n*sm))-(n+1)/n;}var lz=[{x:0,y:0}],cmv=0,tv=vl.reduce(function(a,b){return a+b;},0)||1;vl.forEach(function(v,i){cmv+=v;lz.push({x:(i+1)/n,y:cmv/tv});});
    CH.wpGiniChart=new Chart(document.getElementById('wpGiniChart').getContext('2d'),{type:'line',data:{datasets:[{label:'로렌츠 커브',data:lz,borderColor:c,backgroundColor:hRgba(c,0.1),borderWidth:2,fill:true,tension:0,pointRadius:0},{label:'완전균등선',data:[{x:0,y:0},{x:1,y:1}],borderColor:'#94a3b8',borderDash:[4,4],borderWidth:1,fill:false,tension:0,pointRadius:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},datalabels:{display:false}},scales:{x:{type:'linear',min:0,max:1,ticks:{maxTicksLimit:5,callback:function(v){return (v*100).toFixed(0)+'%';}}},y:{min:0,max:1,ticks:{maxTicksLimit:5,callback:function(v){return (v*100).toFixed(0)+'%';}}}}}});
...
function renderAdvancedMetrics(d,da,col,pf){
    try {
        ...
        var cr = calcStandaloneMetrics(d) || {ot:0, shannon:0, hhi:0, cv:0, hurst:0.5, jaccard:0};
        ...
        var teamRawCvInsight = [0,0,0,0,0,0];
        ...
        if (pf === 'wp' || pf === 'pj') {
            var teamData = filtered().filter(function(r){return r.project==='경남 서부의료원';});
            var teamCr = calcStandaloneMetrics(teamData) || {ot:0, shannon:0, hhi:0, cv:0, hurst:0.5, jaccard:0};
            ...
            teamRawCvInsight = [Math.max(0, 1 - (teamCr.ot||0)), (teamCr.shannon||0), (teamCr.hhi||0), teamCr.cv||0, teamCr.hurst||0, teamCr.jaccard||0];
        ...
        if(document.getElementById(pf + 'InsightRadar')) {
            CH[pf + 'InsightRadar'] = new Chart(document.getElementById(pf + 'InsightRadar').getContext('2d'), {
                ...
                options: {
                    ...
                    plugins: {
                        legend: { display: false },
                        datalabels: { display: function(cx) { return cx.datasetIndex === 0; }, formatter: function(v) { return Number(v).toFixed(2); }, color: col, font: { size: 10, weight: 'bold' }, anchor: 'end', align: 'end' },
                        tooltip: { callbacks: { label: function(cx) { if(cx.datasetIndex === 0) return cx.dataset.label + ': ' + Number(rawCvInsight[cx.dataIndex]).toFixed(2); else return cx.dataset.label + ': ' + Number(teamRawCvInsight[cx.dataIndex]).toFixed(2); } } }
                    },
                    scales: { r: { min: 0, max: 1.2, ticks: { display: false }, pointLabels: { font: {size:10, weight:'800'}, color: '#64748b', padding: 15 } } }
                }
            });
        }
        ...
        wk.forEach(function(k,i){
            ...
            ky.forEach(function(sub){var p=sm[sub]/tot; hi+=p*p; if(p>0)sn-=p*Math.log(p);}); if(N>1) sn=sn/Math.log(N); else sn=0;
            var personDays = {}; g.r.forEach(r => { var pk = r.date + '|' + r.name; personDays[pk] = (personDays[pk] || 0) + r.min; });
            var os = 0; Object.values(personDays).forEach(m => { var h_val = Math.max(0, (m / 60) - 8); if(h_val > 0) os += (Math.exp(3 * (h_val / 4)) - 1) / (Math.exp(3) - 1); });
            var oi = Object.keys(personDays).length > 0 ? os / Object.keys(personDays).length : 0;
            sd.push(sn); hd.push(hi); od.push(Math.max(0, 1 - oi));
        ...
        function dmc(id, l, xl, da, sa, cx) {
            ...
            } catch(e) { console.error("Line chart render err:", id, e); }
        }
        dmc(pf+'OtChart','정시업무',wl,od,so,'#ef4444'); dmc(pf+'ShannonChart','Shannon',wl,sd,ss,'#14b8a6'); dmc(pf+'HhiChart','HHI',wl,hd,sh,'#f59e0b'); dmc(pf+'CvChart','CV(Norm)',bwl,cd,sc,'#8b5cf6'); dmc(pf+'HurstChart','Hurst',bwl,hud,shu,'#ec4899'); dmc(pf+'JaccardChart','Jaccard',bwl,jd,sj,'#3b82f6');
    } catch(e) { console.error("Advanced Metrics 전체 렌더링 중 오류 발생", e); }
}

// --- 5. Personal KPI 탭 렌더링 ---
    ...
    setTimeout(() => {
        try {
            if(sortedEd.length > 0) {
                ...
                CH['newKpiTrendChart'] = new Chart(document.getElementById('newKpiTrendChart').getContext('2d'), { type: 'line', data: { labels: trendLabels, datasets: [{ data: trendData, borderColor: c, backgroundColor: hRgba(c, 0.1), borderWidth: 2, fill: true, tension: 0.4, pointRadius: ptRadii, pointBackgroundColor: ptBgColors, pointBorderColor: ptBorders, pointBorderWidth: ptBorderWidths, pointHoverRadius: 7 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { display: true, align: 'top', anchor: 'end', offset: 4, color: (cx) => fq.includes(trendLabels[cx.dataIndex]) ? c : '#94a3b8', font: (cx) => ({ size: fq.includes(trendLabels[cx.dataIndex]) ? 12 : 10, weight: fq.includes(trendLabels[cx.dataIndex]) ? '900' : '600' }), formatter: (v) => Number(v).toFixed(1) }, tooltip: { callbacks: { title: function(cx) { return cx[0].label + ' 상세 역량'; }, label: function(cx) { var item = sortedEd[cx.dataIndex]; if(!item) return ''; var lines = ['종합 평균: ' + Number(cx.raw).toFixed(2), '----------------']; RADAR_LABELS.forEach((lbl, i) => lines.push(lbl + ': ' + Number(item.vals[i]).toFixed(1))); return lines; } } } }, scales: { x: { display: false }, y: { display: false, min: Math.max(0, Math.min(...trendData) - 0.5), max: 5.5 } }, layout: { padding: { top: 20, bottom: 10, left: 15, right: 15 } } } });
            }
            CH['newKpiRadarChart'] = new Chart(document.getElementById('newKpiRadarChart').getContext('2d'), { type: 'radar', data: { labels: RADAR_LABELS, datasets: [{ label: '선택 기간 (' + latestQ.q + ')', data: latestQ.vals, borderColor: c, backgroundColor: hRgba(c, 0.15), borderWidth: 2.5, pointBackgroundColor: c, pointBorderColor: '#fff', pointBorderWidth: 1.5, pointRadius: 4, pointHoverRadius: 6, order: 1 }, { label: '팀 전체 평균', data: allEvalAvgVals, borderColor: '#94a3b8', backgroundColor: 'transparent', borderWidth: 1, borderDash: [3, 3], pointRadius: 0, pointHoverRadius: 0, order: 2 }, { label: '개인 평균', data: avgVals, borderColor: '#cbd5e1', backgroundColor: 'rgba(148, 163, 184, 0.08)', borderWidth: 1.5, borderDash: [], pointBackgroundColor: '#cbd5e1', pointRadius: 2, pointHoverRadius: 4, order: 3 }] }, options: { responsive: true, maintainAspectRatio: false, clip: false, layout:{ padding: 10 }, plugins:{ legend:{ display:false }, datalabels: { display: false }, tooltip: { backgroundColor: 'rgba(15,23,42,0.9)', titleFont: { size: 14 }, bodyFont: { size: 13, weight: 'bold' }, padding: 12, callbacks: { label: function(cx) { return cx.dataset.label + ': ' + Number(cx.raw).toFixed(2); } } } }, scales: { r: { min: 0, max: 5, grid: { circular: false, color: 'rgba(203,213,225,0.4)', lineWidth: 1.5 }, angleLines: { display: false }, ticks: { display: false, stepSize: 1 }, pointLabels: { font: { size: 14, weight: '800' }, color: '#475569', padding: 20 } } }, elements: { line: { tension: 0.35 } } } });
            if(topTasksArr.length > 0) { CH['newKpiDonutChart'] = new Chart(document.getElementById('newKpiDonutChart').getContext('2d'), { type: 'doughnut', data: { labels: topTasksArr.map(t => t[0]), datasets: [{ data: topTasksArr.map(t => mToH(t[1])), backgroundColor: SUB_PAL.slice(0, topTasksArr.length), borderWidth: 2, borderColor: '#fff' }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '70%', plugins: { legend: { display: false }, datalabels: { display: false } }, layout: { padding: { top: 5, bottom: 5, left: 5, right: 5 } } } }); }
            CH['kpiInsightRadar'] = new Chart(document.getElementById('kpiInsightRadar').getContext('2d'), { type: 'radar', data: { labels: INSIGHT_RADAR_LABELS, datasets: [{ label: latestQ.q, data: cvInsight, borderColor: c, backgroundColor: hRgba(c, 0.15), borderWidth: 2, pointBackgroundColor: c, pointBorderColor: '#fff', pointBorderWidth: 1, pointRadius: 3, pointHoverRadius: 5, order: 1 }, { label: '팀 전체 평균', data: teamCvInsight, borderColor: '#cbd5e1', backgroundColor: 'transparent', borderWidth: 1.5, borderDash: [4, 4], pointRadius: 0, pointHoverRadius: 0, order: 2 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, datalabels: { display: false }, tooltip: { backgroundColor: 'rgba(15,23,42,0.9)', titleFont: { size: 12 }, bodyFont: { size: 11, weight: 'bold' }, padding: 10, callbacks: { label: function(cx) { if(cx.datasetIndex===0) return cx.dataset.label + ': ' + Number(rawCvInsight[cx.dataIndex]).toFixed(2); else return cx.dataset.label + ': ' + Number(teamRawCvInsight[cx.dataIndex]).toFixed(2); } } } }, scales: { r: { min: 0, max: 1.2, grid: { color: 'rgba(203,213,225,0.4)', lineWidth: 1 }, angleLines: { color: 'rgba(203,213,225,0.4)' }, ticks: { display: false, stepSize: 0.3 }, pointLabels: { font: { size: 9, weight: '800' }, color: '#64748b', padding: 8 } } } } });
